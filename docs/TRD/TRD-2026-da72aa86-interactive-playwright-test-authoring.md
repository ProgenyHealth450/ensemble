...
This TRD translates `PRD-2026-da72aa86` into an implementation plan for a new `/ensemble:author-playwright-tests` command: an interactive, post-implementation Playwright test-authoring session for a QA engineer and the application's developers. The session runs once a story's implementation is complete and its PR is open, grounds each proposed test in both the PRD's ACs and the real implementing code, runs confirmed tests against the application's QA environment (headed or headless, the QA engineer's choice), places them in the application's existing E2E test project per existing convention, syncs their steps to an Azure DevOps Test Case/Suite as plain-English steps, and files ADO tasks for any AC the implementation doesn't actually satisfy.
...
[author-playwright-tests.yaml] (orchestrator)
    ...
    - scan the application's E2E test project for @hash:/@ado-testcase: tags (resume-scan.js) -> skip already-confirmed ACs
...
| ADO test plan resolver | `packages/e2e-testing/lib/ado-test-plan.js` | Resolve/create the project's Test Plan every Test Suite call is scoped to (TRD-038) — never auto-picks or auto-names one |
...
- [x] **TRD-005**: Implement resume-detection scan of the application's E2E test project spec files for per-AC `@hash:` tags (4h) [satisfies REQ-011] [depends: TRD-002]
...
**Shippable State:** The QA engineer can run a full interactive session against a story with an open PR and get real, human-confirmed Playwright tests landed in the application's E2E test project for every AC — choosing headed or headless mode, accepting/revising/rejecting proposals, and marking non-automatable ACs manual — without ADO sync or gap-filing yet.
...
- [x] **TRD-011**: Extend `@playwright-tester` to run a proposed test headed (the QA engineer's interactive Entra login) or headless (`app-e2e-auth-state.json`) per session mode (5h) [satisfies REQ-005] [satisfies REQ-013] [depends: TRD-008]
  ...
  - Implementation AC:
    - Given headless mode, when a test runs, then it authenticates via the existing `app-e2e-auth-state.json` secure file.
...
- [x] **TRD-014**: Implement test placement into the application's E2E test project — correct base class, `[TestCategory("E2E")]`, `TestConfiguration.*`, append-to-existing-file behavior (4h) [satisfies REQ-006] [depends: TRD-010]
...
- [x] **TRD-016**: Resolve or create the story's Test Suite, linked to the PRD's referenced work item (4h) [satisfies REQ-007] [depends: TRD-014]
...
**Shippable State:** Running `/ensemble:author-playwright-tests` in a repo hosted on Azure DevOps Repos correctly detects an open PR and proceeds, instead of always halting with "no open PR" regardless of actual PR state.
...
**Shippable State:** Grounding a PR in the consuming application diffs against its real target branch (e.g. `integration`), not a hardcoded `main` guess, and the Azure DevOps trigger check actually recognizes an open PR against the live MCP server's response shape, not just a hand-written test double.
...
### PR 11: Test Plan Sync Never Silently Assumes a Plan or Suite Already Exists

**Shippable State:** Running the Azure DevOps Test Plan Sync phase against a project with zero existing Test Plans no longer gets permanently stuck — the QA engineer is asked to pick an existing Test Plan or name a new one, and is asked to confirm before a new Test Suite is created rather than the tool silently assuming "create" is always right.

- [x] **TRD-038**: Resolve or create the project's Test Plan every Test Suite call is scoped to, never auto-picking or auto-naming one (4h) [satisfies REQ-007] [depends: TRD-016]
  - Target Files: `packages/e2e-testing/lib/ado-test-plan.js` (new), `packages/e2e-testing/commands/author-playwright-tests.yaml`, generated `packages/e2e-testing/commands/ensemble/author-playwright-tests.md`
  - Implementation AC:
    - Given a project with zero existing Test Plans, when Phase 5 Step 1 runs, then the orchestrator asks the QA engineer to name a new plan, rather than assuming one exists or silently creating one with an invented name.
    - Given a project with one or more existing Test Plans, when Phase 5 Step 1 runs, then the orchestrator presents the fetched list and asks the QA engineer to pick one or name a new one — never auto-selecting the only plan just because there is only one.
    - Given the QA engineer's choice, when `resolveOrCreateTestPlan()` runs, then it returns a `resolve` decision for a selected plan or a `create` decision for a named one, and throws if given neither, both, or a `selectedPlanId` that doesn't match any fetched plan.

- [x] **TRD-039**: Thread the resolved Test Plan id through Test Suite resolution and let a QA-engineer-selected suite override the automatic match (4h) [satisfies REQ-007] [depends: TRD-038]
  - Target Files: `packages/e2e-testing/lib/ado-test-suite.js`, `packages/e2e-testing/commands/author-playwright-tests.yaml`, generated `packages/e2e-testing/commands/ensemble/author-playwright-tests.md`
  - Implementation AC:
    - Given a resolved `planId`, when `resolveOrCreateTestSuite()` runs, then every returned `resolve`/`create` decision carries it, and the call throws if `planId` is missing — a Test Suite is never resolved/created without a known parent Test Plan.
    - Given no automatic workItemId/name match is found, when Phase 5 Step 2 runs, then the orchestrator confirms with the QA engineer before creating a new suite, offering the fetched `existingSuites` as an alternative — never treating "no match" as silent permission to create.
    - Given the QA engineer picks an existing suite instead of creating a new one, when `resolveOrCreateTestSuite()` is re-called with `selectedSuiteId`, then it resolves to that suite, overriding whatever the automatic workItemId/name match would have produced.
...
>
> **v1.3.0 addendum:** PR 7 (TRD-032/TRD-032-TEST) adds 2 tasks / 7h, found live-dogfooding PR 6 against a real, open PR in the consuming application before PR 6 had even merged. `backend-developer` fits, same as PR 6.
>
> **v1.4.0 addendum:** PR 8 (TRD-033/TRD-033-TEST) adds 2 tasks / 9h, found live-testing the actual installed plugin against a real, open PR in the consuming application. `backend-developer` fits.
>
> **v1.5.0 addendum:** PR 9 (TRD-034 through TRD-036 + a combined TEST task) adds 4 tasks / 17h, found live-dogfooding this feature against a real, open PR in the consuming application. `backend-developer` fits — plain Node.js library logic (grounded-marker-checker.js, deriveAuthStatePath) and orchestrator/agent prose, not Playwright execution itself.
>
...
2. **Issue:** The ADO Test Plan sync (PR 3) and gap-task filing (PR 4) both assume a linkable ADO work item — but this repo itself has no ADO tracking.
   **Resolution:** Confirmed with the user: this TRD's own id is a micro-UUID like its source PRD (no ADO work item for the ensemble repo's own development); the ADO sync/gap-filing *feature* this tool builds targets the consuming application's ADO project, since that's where the Stories being tested actually live.
...
6. **Issue (found live-dogfooding PR 9, v1.6.0):** even after TRD-036, `test-runner-mode.js` still modeled mode and auth strategy as the same choice: headed always meant a live interactive login with no stored credentials; headless always meant a stored auth-state file. Real harnesses behind SSO (not unique to any one consuming application) capture one stored auth state once and reuse it for every run, headed or headless alike — headed/headless there only toggles whether a human is watching. The contract's documented model never reflected this, resurfacing the same thread as an earlier "auth state file not found" blocker.
...
- PR 6 (TRD-031/TRD-031-TEST): implemented directly on the same branch/PR #10, verified against a real Azure DevOps remote URL and its own test suite.
- PR 7 (TRD-032/TRD-032-TEST): implemented directly on the same branch/PR #10, found and verified live-dogfooding PR 6 against a real, open PR in the consuming application.
- PR 8 (TRD-033/TRD-033-TEST): implemented directly on the same branch/PR #10, found and verified live-testing the actual installed plugin against a real, open PR in the consuming application.
