...
[NEEDS CLARIFICATION: Should the validate-side walk cover `packages/pi/agents/*.md` and `packages/codex/**` too? Both parse today, but they are generated by different pipelines with their own escaping. Covering them makes the check the single repo-wide guarantee; scoping it to `scripts/lib`'s own output keeps ownership clean.]
...
| REQ-005 | CI enforces the same check | Must | Low | 2 |
...
|---|---|---|
| Completeness | 4.5 | All three defect classes covered and the root cause generalized past them. One open clarification on validate-side scope. |
