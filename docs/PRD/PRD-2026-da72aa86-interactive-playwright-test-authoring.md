...
**Target users:** QA engineer and the implementing developer, as the two sides of the interactive session.
...
**Accepted v1 risk:** REQ-008 and the manual/gap-flagging paths (REQ-009, REQ-017) assume a future "refine tests" tool will eventually give the QA engineer a place to resolve unsynced or flagged items. That tool does not exist yet. If it is never built, those items have no resolution path beyond what's noted in this PRD. This is accepted as a known v1 gap, not something this effort solves.

## User Analysis

- **QA engineer**: drives the interactive session, reviews/accepts/revises/rejects each proposed test, confirms pass/fail, decides when an AC is manual-only, confirms real AC gaps.
...
- AC-003-2: Given the QA engineer requests changes to a proposed test, when they describe the change, then the agent revises the test and re-presents it rather than silently finalizing the original.
- AC-003-3: Given the QA engineer rejects a proposed test outright (not a request for changes, but declining the approach entirely), when this happens, then the AC falls into REQ-017's manual/not-automatable escape hatch rather than being left silently uncovered.
...
- AC-004-2: Given the QA engineer stops at a checkpoint, when the session ends, then progress made so far is preserved for a later resume (per REQ-011).

### REQ-017: Manual / Not-Automatable AC Escape Hatch
**Priority:** Should | **Complexity:** Low

- AC-017-1: Given an AC that cannot reasonably be automated (e.g. a visual/manual check), when the QA engineer marks it as manual-only during the walkthrough, then the session records it as manual-only — distinct from both a confirmed test and an AC gap — rather than dropping it silently or forcing a test.
...
- AC-005-1: Given a test the QA engineer has accepted, when the session runs it against the QA environment, then its pass/fail result is shown to the QA engineer before the session moves to the next AC.
- AC-005-2: Given a confirmed test fails on first run, when this happens, then the agent investigates (selector/timing/data issue), fixes and reruns, or surfaces the failure to the QA engineer as a blocker for that AC.
...
- AC-009-2: Given an AC gap is flagged, when the QA engineer reviews it, then they can confirm the gap is real (triggering REQ-010) or override it as a false read by pointing the agent at the correct code path.
...
- AC-016-1: Given a session runs, when any action occurs (test written, run result, sync result, gap task filed, manual-AC marked), then it's logged as human-readable console output a human can review after the session ends — v1 targets the QA engineer watching live, not a downstream consumer, so no structured (JSON) format is required.
