...
they haven't seen yet.
   ...
   - Build a request {acText, groundingDiff} and validate it with validateProposalRequest(req) from packages/e2e-testing/lib/delegation-contract.js — acText is this AC's own text (not the whole REQ), groundingDiff is Phase 1's groundImplementation() result for its REQ
   - Delegate the validated request to @playwright-tester (packages/e2e-testing/agents/playwright-tester.md), which grounds the AC and authors a test, plus a plainEnglishSummary of what it does — never running it at this stage
   ...
   - If the response is an authoringFailure with no viable alternative: call markManual(acId, reason) from packages/e2e-testing/lib/manual-ac-tracker.js, log via logAction({type: "manual-ac-marked", acId, reason}), and move to the next AC — never proceed to Phase 4 with nothing to present

   **Delegation:** @playwright-tester
   Ground one AC in code and propose a Playwright test, with a plain-English summary of what it does — do not run it yet, per packages/e2e-testing/lib/delegation-contract.js's Proposal stage.
...
environment run on tests a QA engineer might still reject.
   ...
   - Call recordDecision(decision, {acId, proposedTest, changeDescription, iterationCount}) from packages/e2e-testing/lib/ac-decision-loop.js with exactly one of "accept" | "request-changes" | "reject"
   ...
   - outcome "manual-escape-hatch" (an outright reject): call markManual(acId, reason) from packages/e2e-testing/lib/manual-ac-tracker.js, log via logAction({type: "manual-ac-marked", acId, reason}), and move to the next AC — the test is never run
...
environment-mismatch signal before framing it as one.
   ...
   - Build a request {acText, groundingDiff, proposedTest, targetEnv, mode, authStatePath} and validate it with validateRunRequest(req) from packages/e2e-testing/lib/delegation-contract.js — proposedTest is the exact confirmed source from the previous step (never re-authored), acText/groundingDiff are repeated from Phase 3 step 2 (this is a fresh, stateless delegation with no memory of the Proposal stage), targetEnv/mode/authStatePath are Phase 2's resolved values
   ...
   - If the run failed and runResult.environmentMismatchSuspected is true: lead the failure report with buildEnvironmentMismatchHint({markersChecked: runResult.groundedMarkersChecked, markersFound: []}) from packages/e2e-testing/lib/grounded-marker-checker.js — present this as the LEADING hypothesis, before any suggestion that the implementation itself is broken, and prompt the QA engineer to confirm the environment (Phase 2 Step 2's question) before deciding what to do next
...
regression plan), a real decision for the QA engineer to make.
   ...
   - Call resolveOrCreateTestPlan({existingPlans, selectedPlanId?, newPlanName?}) from packages/e2e-testing/lib/ado-test-plan.js with their choice
...
QA-environment-verification fix (TRD-034).
   ...
   - Fetch the story's existing Test Suites within the resolved planId (e.g. via the Azure DevOps MCP server's testplan_list_test_suites-equivalent tool) and call resolveOrCreateTestSuite({planId, workItemId, storyTitle, existingSuites}) from packages/e2e-testing/lib/ado-test-suite.js
