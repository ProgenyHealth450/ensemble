# @sunstone-partners/ensemble-rspec

RSpec testing framework integration

## Installation

```bash
claude plugin install @sunstone-partners/ensemble-rspec
```

## Description

Part of the ensemble plugin ecosystem for Claude Code. This plugin provides rspec testing framework integration.

## Features

- TBD (to be populated during plugin extraction)

## Usage

After installation, this plugin's agents, commands, and skills will be automatically available in Claude Code.

## Documentation

See the [main ensemble repository](https://github.com/Sunstone-Partners/ensemble) for complete documentation.

## License

MIT
