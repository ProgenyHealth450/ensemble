...
3. Detect ask_user availability: set INTERACTIVE=true if available, INTERACTIVE=false otherwise. (This is the same detection this file performs again later in Feature Branch Creation for branch-intent resolution — reused here since PR-backend resolution needs it earlier in Preflight; not a new detection mechanism.)
...
12. Run: git status --porcelain — HALT if output non-empty (dirty working directory) [UNCHANGED]
