...
**4. Git-Town and Working Directory Verification**
   ...
   - Detect AskUserQuestion availability: set INTERACTIVE=true if available, INTERACTIVE=false otherwise. (This is the same detection this file performs again later in Feature Branch Creation for branch-intent resolution — reused here since PR-backend resolution needs it earlier in Preflight; not a new detection mechanism.)
